...
YouEduAppMenu.init = (function(){
    ...
    $('li a').each(function(index, element) {
        var randomColor = YouEduAppMenu.array_colors[Math.floor(Math.random() * YouEduAppMenu.array_colors.length)];
    ...
    $('#mainmenu > ul li.has-sub').hover(function() {
        ...
        numChilds = (numChilds > 1) ? numChilds : 0;
    ...
    $('.menu_icon_image').click(function() {
        if ($(this).hasClass('menu_icon_image_opened')) {
            ...
            YouEduAppMenu.hideMenu(0.15);
        } else {
            $(this).addClass('menu_icon_image_opened');
            YouEduAppMenu.showMenu(0.15);
        }
    });

    $('.menu_icon_admin').click(function() {
        YouEduAppMenu.hideMenu(0.15);
        setTimeout(function() {
            window.location.href = YouEduAppMenu.context + "/admin/";
        }, 1000);
    });
});
...
YouEduAppMenu.home = (function(){
    YouEduAppMenu.showMenu(1.5);
});

YouEduAppMenu.showMenu = (function(delay) {
    YouEduAppMenu.menuFunction('bounceInDown', delay, 1.75);
});
YouEduAppMenu.hideMenu  = (function(delay) {
    YouEduAppMenu.menuFunction('slideOutLeft', delay, 2.75);
});

YouEduAppMenu.menuFunction = (function(animation, delay, duration) {
...
YouEduAppMenu.resizeWindowHideNavBar = (function() {
    var height = $(window).height();
    var perc = 0.1;
    $("#wrapper").animate({height: height + (height * perc)}, 10, function() {
    });
    $('#wrapper').css('height', height + (height * perc));
    $('#wrapper').css('bottom', (height * perc));
    $('#header').css('top', (height * perc));
    window.scrollTo(0, 1);
});
